...
OWNER_ID = 8627605535 # Replace with your Owner ID
ADMIN_ID = 8627605535 # Replace with your Admin ID (can be same as Owner)
YOUR_USERNAME = 'bdasifbro' # Replace with your Telegram username (without the @)
UPDATE_CHANNEL = 'https://t.me/new_bangla_mod' # Replace with your update channel link

# Folder setup - using absolute paths
BASE_DIR = os.path.abspath(os.path.dirname(file)) # Get script's directory
...
# 🎯 এডমিন অ্যাপ্রুভালের জন্য পেন্ডিং ফাইল ট্র্যাক করার গ্লোবাল মেমরি ডিকশনারি
pending_approvals = {}
...
def init_db():
    ...
    try:
        ...
        c = conn.cursor()
        c.execute('''CREATE TABLE IF NOT EXISTS subscrip
tions
    ...
    if script_info and script_info.get('process'):
        try:
            ...
            if not is_running:
                ...
                if 'log_file' in script_info and hasattr(script_info['log_file'], 'close') and not script_info['l
og_file'].closed:
                    try:
...
def run_script(script_path, script_owner_id, user_folder, file_name, messa
ge_obj_for_reply, attempt=1):
...
def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        if not os.path.exists(script_path):
             bot.reply_to(message_obj_for_reply, f"❌ Error: Script '{file_name}' not found at '{script_path}'!")
             logger.error(f"JS Script not found: {script_path} for user {script
_owner_id}")
             if script_owner_id in user_files:
                 user_files[script_owner_id] = [f for f in user_files.get(script_owner_id, []) if f[0] != file_name]
             remove_user_file_db(script_owner_id, file_name)
...
def add_active_user(user_id):
    ...
    with DB_LOCK:
        ...
        try:
            c.e
xecute('INSERT OR IGNORE INTO active_users (user_id) VALUES (?)', (user_id,))
            conn.commit()
...
def create_main_menu_inline(user_id):
    ...
    buttons = [
        ...
        types.InlineK
eyboardButton('⚡ Bot Speed', callback_data='speed'),
...
# --- File Upload Handler (With Admin Permission System) ---
...
def handle_docs(message):
    ...
    if bot_locked and user_id not in admin_ids:
        bot.reply_to(message, "🔒 The bot is currently locked by the admin. Only admins can use it right now.")
        return
    ...
    if current_count >= limit:
        bot.reply_to(message, f"❌ Limit reached! You have uploaded {current_count}/{limit} files. Delete old files to upload new ones.")
        return

    file_name = message.document.file_name
    if not (file_name.endswith('.py') or file_name.endswith('.js') or file_name.endswith('.zip')):
        bot.reply_to(message, "❌ Invalid file type! Only .py, .js, and .zip files are
supported.")
        return

    try:
        # Download file to a safe variable structure
        file_info = bot.get_file(message.document.file_id)
        downloaded_file = bot.download_file(file_info.file_path)
        
        # If Admin or Owner uploads, save directly without restriction
        if user_id in admin_ids:
            ...
            file_type = 'zip' if file_name.endswith('.zip') else ('js' if file_name.endswith('.js') else 'py')
            if file_type == 'zip':
                try:
                    with zipfile.ZipFile(file_path, 'r') as zip_ref:
                        zip_ref.extractall(user_folder)
                    os.remove(file_path)
                    bot.reply_to(message, "✅ ZIP file uploaded and extracted successfully!")
                    return
                except zipfile.BadZipFile:
                    bot.reply_to(message, "❌ Failed to extract. Bad ZIP file structure.")
                    if os.path.exists(file_path): os.remove(file_path)
                    return
            
            save_user_file(user_id, file_name, file_type)
            bot.reply_to(message, f"👑 Admin File {file_name} saved directly!")
            
            # Auto-run if python or js
            if file_type == 'py':
        ...
        # --- 🛡️ For Regular Users: Admin Approval Pipeline ---
        request_id = f"req_{user_id}_{int(time.time())}"
        user_folder = get_user_folder(user_id)
        temp_file_path = os.path.join(user_folder, file_name)
        
        # Save file to disk temporarily pending approval
        with open(temp_file_path, 'wb') as f:
            f.write(downloaded_file)
            
        file_type = 'zip' if file_name.endswith('.zip') else ('js' if file_name.endswith('.js') else 'py')
        
        # Register request data in global memory tracker
        ...
        bot.reply_to(message, "⏳ Your file has been received! Waiting for ASIF BOSS to approve it.")
        
        # Create Inline Buttons for Admin decision tree
        admin_markup = types.InlineKeyboardMarkup(row_width=2)
        approve_btn = types.InlineKeyboardButton("🟢 Approved", callback_data=f"approve_req_{request_id}")
        delete_btn = types.InlineKeyboardButton("🗑️ Delete System", callback_data=f"delete_req_{request_id}")
        admin_markup.add(approve_btn, delete_btn)
        
        request_msg = (
            f"🚨 New File Permission Requested!\n\n"
            f"👤 User ID: {user_id}\n"
            f"📄 File Name: {file_name}\n"
            f"⚙️ Type: {file_type.upper()}\n\n"
            f"ASIF BOSS, do you want to approve and run this script?"
        )
        
        # Alert the Owner/Admin directly
        bot.send_message(OWNER_ID, request_msg, parse_mode="Markdown", reply_markup=admin_markup)
        
    except Exception as e:
        logger.error(f"Error handling document upload: {e}", exc_info=True)
        bot.reply_to(message, f"❌ Error processing upload request: {e}")

# --- Callback Query Handler for Admin Permission Tree and Core Actions ---
@bot.callback_query_handler(func=lambda call: True)
def handle_all_callbacks(call):
    user_id = call.from_user.id
    
    # 1. Catch Permission Approvals first
    if call.data.startswith('approve_req_') or call.data.startswith('delete_req_'):
        if user_id not in admin_ids:
            bot.answer
_callback_query(call.id, "❌ You do not have admin permissions!", show_alert=True)
            return
            
        data_parts = call.data.split('_')
        action = data_parts[0] # 'approve' or 'delete'
        request_id = f"req_{data_parts[2]}_{data_parts[3]}"
        
        if request_id not in pending_approvals:
            bot.answer_callback_query(call.id, "⚠️ Request not found or already processed.", show_alert=True)
            return
        ...
        user_chat = req['chat_id']
        user_folder = get_user_folder(tgt_uid)
            ...
            if file_type == 'zip':
                try:
                    with zipfile.ZipFile(file_path, 'r') as zip_ref:
                        zip_ref.extractall(user_folder)
                    os.remove(file_path)
                    bot.edit_message_text(f"✅ Approved: ZIP file extracted for user {tgt_uid}.", 
                                          chat_id=call.message.chat.id, message_id=call.message.message_id)
                    try: bot.send_message(user_chat, f"🎉 Your ZIP file {file_name} was approved and extracted!")
                    except Exception: pass
                except zipfile.BadZipFile:
                    bot.edit_message_text(f"❌ Error: Extraction failed on invalid ZIP.", 
                                          chat_id=call.message.chat.id, message_id=call.message.message_id)
                    if os.path.exists(file_path): os.remove(file_path)
            else:
                save_user_file(tgt_uid, file_name, file_type)
                bot.edit_message_text(f"🟢 Approved & Running: {file_name} for user {tgt_uid}.", 
                                      chat_id=call.message.chat.id, message_id=call.message.message_id)
                try: bot.send_message(user_chat, f"🎉 Your file {file_name} has been approved and started running!")
                except Exception: pass
                
                # Execute long-running process mapping
                if file_type == 'py':
                    threading.Thread(target=run_script, args=(file_path, tgt_uid, user_folder, file_name, call.message)).start()
                elif file_type == 'js':
                    threading.Thread(target=run_js_script, args=(file_path, tgt_uid, user_folder, file_name, call.message)).start()
                    
        elif action == 'delete':
            if os.path.exists(file_path):
                os.remove(file_path)
            bot.edit_message_text(f"🗑️ Deleted System: {file_name} rejected and cleaned from system storage.", 
                                  chat_id=call.message.chat.id, message_id=call.message.message_id)
            try: bot.send_message(user_chat, f"❌ Your file {file_name} was rejected and deleted by the admin.")
            except Exception: pass
    ...
    # 2. General application fallback mappings for checking/stopping scripts
    # (Rest of existing inline engine callbacks)
...
: logger.error(f"Polling ConnectionError: {ce}. Retrying in 15s..."); time.sleep(15)
        except Exception as ex: logger.error(f"Critical error in polling: {ex}"); time.sleep(10)